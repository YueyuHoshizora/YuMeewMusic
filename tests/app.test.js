...
test('editor initializes, switches formats and reaches download for every format', async () => {
  const downloads = [], encoded = [], elements = new Map();
  function element() {
    return {
      value: '', textContent: '', disabled: false, hidden: false, currentTime: 0,
      style: {}, dataset: {}, children: [], listeners: {},
      classList: { toggle() {} },
      setAttribute() {}, append(...children) { this.children.push(...children); }, remove() {},
      querySelector() { return element(); },
      addEventListener(type, handler) { this.listeners[type] = handler; },
      pause() {}, click() { if (this.download) downloads.push(this.download); },
    };
  }
  ...
  const context = vm.createContext({
    document: { getElementById: id => elements.get(id), querySelectorAll: () => [], createElementNS: element, createElement: element, body: element() },
    window: { addEventListener() {} },
    requestAnimationFrame() {}, setTimeout() {}, AbortController,
    URL: { createObjectURL: () => 'blob:local-test', revokeObjectURL() {} },
    AudioContext: class { async decodeAudioData() { return buffer; } async close() {} },
    getFormat, exportFilename, loadSettings: () => ({ ...DEFAULT_SETTINGS }), saveSettings() {}, applyTheme() {}, draw() {},
    encodeMedia: async options => { encoded.push(options.format); return new Blob(['test']); },
  });
  ...
  await vm.runInContext('loadAudio({ name: "song.wav", size: 100, arrayBuffer: async () => new ArrayBuffer(0) })', context);
