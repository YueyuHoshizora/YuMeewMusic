...
test("影片編輯 exposes a movable dynamic layer and locked text overlays", () => {
  ...
  assert.match(script, /drawDynamicLayer\(canvas, sourceTime/);
  assert.doesNotMatch(script, /drawOverlays\(context, sourceTime\)/);
  assert.match(script, /saveSettings\(\{ \.\.\.settings, style: 19 \}\)/);
