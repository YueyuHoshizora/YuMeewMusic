...
test("video generator page provides a model-ready generation workspace", () => {
  ...
  assert.match(html, /class="video-credits"[^>]*aria-labelledby="video-credits-title"[\s\S]*企劃[\s\S]*href="https:\/\/www\.youtube\.com\/@YuStellar"[^>]*>星語<\/a>[\s\S]*技術指導[\s\S]*href="https:\/\/www\.youtube\.com\/@oktrf2000"[^>]*>修卡迪特<\/a>[\s\S]*程式開發[\s\S]*Yueyu Hoshizora[\s\S]*開源專案[\s\S]*Mediabunny[\s\S]*ONNX Runtime Web[\s\S]*專案網站[\s\S]*the-music\.app[\s\S]*GitHub[\s\S]*YueyuHoshizora\/YuMeewMusic/);
  ...
  assert.match(script, /const STORYBOARD_PRIMARY_URL = "https:\/\/storyboard-checker\.plain-leaf-e871\.workers\.dev\/api\/storyboard\/check"/);
  assert.match(script, /const STORYBOARD_FALLBACK_URL = "https:\/\/inspiration-chat\.plain-leaf-e871\.workers\.dev\/api\/storyboard\/check"/);
  ...
  assert.match(script, /https:\/\/model-proxy\.plain-leaf-e871\.workers\.dev\/minimax\/video/);
  ...
  assert.match(script, /https:\/\/model-proxy\.plain-leaf-e871\.workers\.dev\/byteplus\/video/);
  ...
  assert.match(script, /GOOGLE_VIDEO_PROXY_URL = "https:\/\/model-proxy\.plain-leaf-e871\.workers\.dev\/google\/video"/);
  ...
  assert.match(script, /RESOURCE_UPLOAD_URL = "https:\/\/model-proxy\.plain-leaf-e871\.workers\.dev\/resources\/upload"/);
...
test("storyboard AI analysis prefers storyboard-checker and falls back to inspiration-chat, but never retries a 429 against the fallback", () => {
  const script = readFileSync("js/video-generator.js", "utf8");
