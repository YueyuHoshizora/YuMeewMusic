...
| MV 動畫（AI 生成 2.5D 場景，選用） | 分鏡的場景描述、鏡頭語言、景別、氛圍等文字；不含音樂、參考圖或參考人物 | 依文字生成分層插畫規格供瀏覽器繪製動畫畫面；由 inspiration-chat Worker（代理 OpenRouter）處理，不會保存內容。AI 無法使用時瀏覽器改用本機關鍵字模板，不送出任何資料 |
