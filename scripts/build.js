...
for (const path of ["index.html", "subtitle-editor.html", "converter.html", "video-editor.html", "favicon.svg", "css", "js", "vendor"])
  await cp(path, `dist/${path}`, { recursive: true });
// A single content-derived version keeps entry points and app modules in sync after deploys.
const htmlFiles = ["index.html", "subtitle-editor.html", "converter.html", "video-editor.html"];
const files = [...htmlFiles, "css/style.css", "css/subtitle-editor.css", "css/converter.css", "css/video-editor.css", ...(await readdir("js")).filter(name => name.endsWith(".js")).sort().map(name => `js/${name}`)];
