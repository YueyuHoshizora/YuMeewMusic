...
主畫面頂部的「文生影」會開啟獨立的 AI 影片生成頁面。介面比照文生圖，包含題詞詞語、影片描述、Prompt 轉譯、模型、API KEY、影片預覽、下載及套用主畫面背景等區域。模型選單包含 MiniMax H3（`MiniMax-H3`）及 MiniMax H3 Max（`MiniMax-H3-Max`），可分別保存 API KEY，也能從其他模型複製既有金鑰。前端依 MiniMax Video Generation V2 規格建立任務並輪詢結果：H3 可選 768P／2K 與 4～15 秒，H3 Max 可選 480P／768P 與 5～15 秒，兩者皆提供 21:9、16:9、4:3、1:1、3:4、9:16。生成完成後可下載 MP4；瀏覽器能讀取影片檔案時也可保存到 IndexedDB 並套用為主畫面背景素材。
