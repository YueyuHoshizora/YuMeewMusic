...
function populateFormats() {
  ...
  $('converter-format').replaceChildren(...formats.map(format => {
    ...
    option.value = format;
    option.textContent = t(CONVERTER_FORMAT_LABELS[format]);
    return option;
  }));
...
function describeTracks(info) {
  ...
  if (info.audio) parts.push(`${info.audio.codec.toUpperCase()} · ${info.audio.channels} ${t('聲道')} · ${Math.round(info.audio.sampleRate / 1000)} kHz`);
  return parts.join(' ＋ ');
}
...
async function loadFile(file) {
  ...
  if (file.size > MAX_FILE_SIZE) {
    error(t('檔案超過 1 GB，為避免瀏覽器記憶體不足，請選擇較小的檔案。'));
    return;
  }
  $('converter-file-name').textContent = file.name;
  $('converter-file-help').textContent = t('正在辨識影片與音訊軌…');
  ...
  try {
    ...
    sourceFile = file;
    sourceInfo = info;
    $('converter-kind').textContent = t(info.kind === 'video' ? '影片輸入' : '音樂輸入');
    ...
    $('converter-file-help').textContent = t('點擊可更換檔案');
    populateFormats();
    status(t(info.kind === 'video' ? '已辨識為影片，請選擇輸出格式。' : '已辨識為音樂，請選擇輸出格式。'), 'success');
    $('converter-file-help').textContent = 'MP4 · MOV · WebM · MP3 · M4A · FLAC · WAV';
    error(cause.message || t('無法讀取這個檔案，請確認格式是否受瀏覽器支援。'));
  } finally {
  ...
  try {
    const blob = await convertMediaFile({
      ...
      onProgress(value) {
        ...
        $('converter-progress-text').textContent = t('轉換進度 {0}%', value);
        $('converter-start').textContent = t('正在轉換 {0}%', value);
      },
    });
    ...
    status(t('{0} 轉換完成，下載已開始。', format.toUpperCase()), 'success');
  } catch (cause) {
    const text = cause.message || t('轉換失敗，這個來源編碼可能不受目前瀏覽器支援。');
    if (/取消/.test(text)) status(t('已取消轉換。'));
    else error(text);
  } finally {
    controller = null;
    $('converter-drop').disabled = false;
    $('converter-format').disabled = false;
    $('converter-start').disabled = false;
    $('converter-start').textContent = t('開始轉換');
