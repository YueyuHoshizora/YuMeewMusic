...
主畫面頂部的「文生影」會開啟獨立的 AI 影片生成頁面。初版介面比照文生圖，包含題詞詞語、影片描述、Prompt 轉譯、模型、API KEY、16:9 影片預覽、下載及套用主畫面背景等區域。模型選單包含 MiniMax H3（`MiniMax-H3`）及 MiniMax H3 Max（`MiniMax-H3-Max`），可分別保存 API KEY，也能從其他模型複製既有金鑰；生成 API 待後續串接。
