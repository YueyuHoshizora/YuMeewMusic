...
function syncGenerateAvailability() {
  ...
  $("export-video-project").disabled = busy || !autoDraftReady || storyboards.size === 0;
  $("select-video-project").disabled = busy || !autoDraftReady;
  $("clear-video-resources").disabled = busy || !autoDraftReady || storyboards.size === 0;
}
...
// ---- 草稿自動儲存（分鏡＋全片風格＋輸出設定，跟影片生成器同一套節流／IndexedDB 模式）----
...
function outputSettings() {
  return {
    ...
    resolution: $("mv-resolution").value,
    fps: $("mv-fps").value,
    format: $("mv-format").value,
  };
}
function autoDraftMetadata() {
  return {
    filmStyle,
    storyboards: orderedStoryboardEntries().map((entry) => entry.draft),
...
function saveAutoDraftNow() {
  ...
  const operation = async () => {
    ...
    await saveStoredValue(DRAFT_KEY, draft);
    setAutoDraftStatus(`草稿已自動儲存 · ${draftTimeLabel(savedAt)}`);
    return draft;
  };
  autoDraftSavePromise = autoDraftSavePromise.then(operation, operation).catch((saveError) => {
    setAutoDraftStatus("草稿自動儲存失敗", "error");
    throw saveError;
  });
  return autoDraftSavePromise;
}
function restoreOutputSettings(saved = {}) {
  settings.darkness = Number.isFinite(Number(saved.darkness)) ? Number(saved.darkness) : 0;
  $("mv-darkness").value = String(settings.darkness);
  $("mv-darkness-output").textContent = String(settings.darkness);
  if (saved.aspectRatio) $("mv-aspect-ratio").value = saved.aspectRatio;
  if (saved.resolution) $("mv-resolution").value = saved.resolution;
  if (saved.fps) $("mv-fps").value = saved.fps;
  if (saved.format) $("mv-format").value = saved.format;
...
async function restoreAutoDraft() {
  try {
    const draft = await loadStoredValue(DRAFT_KEY);
    if (autoDraftTouched) {
    ...
    if (!metadata || !Array.isArray(metadata.storyboards)) {
      finishAutoDraftRestore();
      setAutoDraftStatus("自動儲存已啟用");
      return;
    }
    filmStyle = { ...EMPTY_FILM_STYLE, ...(metadata.filmStyle || {}) };
    ...
    storyboards.clear();
    metadata.storyboards.forEach((draftItem) => storyboards.set(draftItem.id, draftItem));
    $("video-prompt").replaceChildren(...[...storyboards.values()].map(createStoryboardBlock));
    restoreOutputSettings(metadata.settings);
...
// ---- 生成歷史（最多保留 10 個版本，跟影片生成器同一套 IndexedDB 模式）----
...
async function loadGenerationHistory() {
  const saved = await loadStoredValue(HISTORY_KEY).catch(() => null);
  generationHistory = Array.isArray(saved?.items) ? saved.items.filter((item) => item?.blob?.size).slice(0, HISTORY_LIMIT) : [];
  syncHistoryButton();
...
async function saveGenerationHistory(blob, metadata) {
  ...
  const record = {
    id: globalThis.crypto?.randomUUID?.() || `mv-${Date.now()}-${Math.random().toString(36).slice(2)}`,
    blob,
    name: exportFilename("mv-animation", metadata.format, metadata.resolution, metadata.fps),
    createdAt: Date.now(),
    ...metadata,
  };
  ...
  while (generationHistory.length) {
    try {
      await saveStoredValue(HISTORY_KEY, { items: generationHistory, updatedAt: Date.now() });
      syncHistoryButton();
      return record;
    } catch (saveError) {
      lastError = saveError;
...
function syncResultHeading(metadata = generatedVideoMetadata) {
  $("result-video-resolution").textContent = metadata?.resolution ? `${metadata.resolution}p` : "動畫";
  $("result-video-ratio").textContent = `${metadata?.ratio || "原始比例"} · ${(metadata?.format || "mp4").toUpperCase()}`;
}
function loadHistoryVersion(id) {
  ...
  syncResultHeading(record);
  presentVideo();
  $("video-history-dialog").close();
  status(`已載入 ${historyTime(record.createdAt)} 的生成版本`, "success");
}
async function deleteHistoryVersion(id) {
  ...
  if (generationHistory.length) await saveStoredValue(HISTORY_KEY, { items: generationHistory, updatedAt: Date.now() });
  else await deleteStoredValue(HISTORY_KEY);
  syncHistoryButton();
  renderVideoHistory();
}
function renderVideoHistory() {
  ...
  for (const record of generationHistory) {
    ...
    title.textContent = "動畫版本";
    const meta = document.createElement("small");
    meta.textContent = `${historyTime(record.createdAt)} · ${record.resolution || ""}p · ${record.ratio || ""} · ${record.fps || ""} fps`;
...
function compareSelectedVideos() {
  ...
  grid.replaceChildren(...records.map((record) => {
    ...
    meta.textContent = `${historyTime(record.createdAt)} · ${record.resolution || ""}p · ${record.ratio || ""}`;
    item.append(video, meta);
    return item;
  }));
  $("video-compare-dialog").showModal();
}
async function restoreLastGeneratedAnimation() {
  if (lastGeneratedVideoRestored || busy) return;
  lastGeneratedVideoRestored = true;
  const latest = generationHistory[0];
  if (!latest?.blob?.size) return;
  releaseVideo();
  generatedVideoBlob = latest.blob;
  generatedVideoUrl = URL.createObjectURL(generatedVideoBlob);
  generatedVideoMetadata = latest;
  syncResultHeading(latest);
  presentVideo();
  status("已載入上次生成結果", "success");
}
...
async function generateVideo() {
  ...
  try {
    ...
    const metadata = { resolution, ratio: $("mv-aspect-ratio").value, fps, format, duration };
    const historyRecord = await saveGenerationHistory(blob, metadata).catch(() => null);
    generatedVideoMetadata = historyRecord || metadata;
    syncResultHeading(generatedVideoMetadata);
    presentVideo();
    status(historyRecord ? "動畫已完成，可在下方播放區查看或下載。" : "動畫已完成，但瀏覽器無法保存生成歷史，仍可在下方播放或下載。", historyRecord ? "success" : "error");
  } catch (exportError) {
    status("生成失敗", "error");
...
for (const id of ["mv-resolution", "mv-fps", "mv-format"]) $(id).addEventListener("change", () => scheduleAutoDraft());
...
$("download-video").addEventListener("click", () => {
  if (busy || !generatedVideoBlob) return;
  const link = document.createElement("a");
  link.href = generatedVideoUrl;
  link.download = generatedVideoMetadata?.name || exportFilename("mv-animation", $("mv-format").value, $("mv-resolution").value, $("mv-fps").value);
  document.body.append(link);
  link.click();
  link.remove();
});
...
window.addEventListener("pagehide", () => {
  if (autoDraftReady) void saveAutoDraftNow().catch(() => {});
});
...
void loadGenerationHistory().then(() => restoreWhenIdle(() => restoreLastGeneratedAnimation()));
