...
test("video character templates preserve their reference images in IndexedDB", async () => {
  ...
  const project = { characters: [{ name: "小雨", referenceImage, style: "動畫", tone: "溫柔", voice: "清亮", clothing: "藍色外套" }], updatedAt: 123 };
  ...
  assert.equal(restored.characters[0].voice, "清亮");
});
...
test("media cache entries identify their page and field and can be cleared", async () => {
  ...
  const generatedVideo = new TestFile(["video-data"], "result.mp4", { type: "video/mp4", lastModified: 789 });
  await saveStoredMedia("audio", audio, database);
  await saveStoredMedia("generated-image", generated, database);
  await saveStoredMedia("generated-video", generatedVideo, database);
  ...
  assert.equal(entries.find(entry => entry.key === "generated-video").page, "影像產生器");
